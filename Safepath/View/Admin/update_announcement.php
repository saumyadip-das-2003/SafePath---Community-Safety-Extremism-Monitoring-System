...
<?php include('../../Model/Admin/update_announcement.php'); ?>
...
<head>
    ...
    <title>Update Announcement - SafePath</title>
...
<body>
    ...
    <div class="content">
        ...
        <?php if (isset($status_message)): ?>
            <div class="status-message">
                <?php if ($status_message == 'success'): ?>
                    <p style="color: green;">Announcement updated successfully!</p>
                <?php elseif ($status_message == 'error'): ?>
                    <p style="color: red;">Error updating announcement. Please try again.</p>
                <?php endif; ?>
            </div>
        <?php endif; ?>

        <!-- Search Form -->
        <form method="GET" action="update_announcement.php" class="search-form">
            <input type="text" name="search" placeholder="Search by title..." value="<?php echo htmlspecialchars($search); ?>" required>
            <button type="submit">Search</button>
        </form>

        <!-- Display error message if no announcement is found -->
        ...
        <?php if (isset($announcement)): ?>
            <form method="POST" action="update_announcement.php" class="update-announcement-form">
                <input type="hidden" name="update_announcement_id" value="<?php echo $announcement['id']; ?>">

                <label for="title">Title:</label>
                <input type="text" name="title" value="<?php echo htmlspecialchars($announcement['title']); ?>" required>

                <label for="description">Description:</label>
                <textarea name="description" rows="4" required><?php echo htmlspecialchars($announcement['description']); ?></textarea>
